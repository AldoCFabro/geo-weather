...
describe(`[GET] ${prefix}/forecast get Weather And Location with city`, () => {
  ...
  test('1 - It should return a 400 error for not sending the city or the ip ', async () => {
    const response = await api.get(`${prefix}/forecast`).send();
  ...
  test('5 - should receive the forecast up to 5 days and your location by city', async () => {
    const response = await api
      .get(`${prefix}/forecast`)
  ...
  test('6 - should receive the forecast up to 5 days and your location by ip', async () => {
    const response = await api.get(`${prefix}/forecast`).query(INPUT_QUERY_PARAMS.VALID_IP).send();
    expect(response.statusCode).toBe(200);
    expect(response.body).toHaveProperty('error', false);
    expect(response.body).toHaveProperty('message');
    expect(response.body).toHaveProperty('data');
